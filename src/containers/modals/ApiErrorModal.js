    ...
    return (
      <Modal show={true} animation={false} onHide={() => this.props.hideModal()}>
        <Modal.Header closeButton>
          <Modal.Title><Glyphicon glyph='exclamation-sign'/> {contents.title}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
    ...
    switch(error){
      case '400': 
        return {
          ...
          message: 'Your request was rejected by the server because it failed validation.',
        }
      case '409': 
        return {
        title: 'Change conflict',
        message: 'It looks like someone has already made changes to this data, so your changes have not been applied.',
        }
      default:
        return {
          title: 'Server error',
